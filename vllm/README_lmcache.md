...
## Run Container
```bash
docker run -itd --net=host --privileged --ipc=host --device=/dev/dri \
  -v /path/to/your/models:/models \
  -e no_proxy=localhost,127.0.0.1 \
  --memory="256G" --name=lmcache_Test --shm-size="16g" \
  --entrypoint /bin/bash llm-scaler-vllm:lmcache
```
...
# KV geometry is read automatically from $MODEL_PATH/$MODEL/config.json at startup.
...
|---|---|---|
| `MODEL_PATH` | `/models` | Model root directory inside the container, backed by a host directory such as `/path/to/your/models`; combined with `MODEL` to form `$MODEL_PATH/$MODEL` |
